    ...
    public class UserToUser
    {
        ...
        public DataTable updateIncorrectAnswerTableUserToUser(List<string> userToUserList, DataTable dt) //update the counter in UserToUser table = numOfCurrectAnswer = numOfCurrectAnswer-1
        {
            foreach (DataRow dr in dt.Rows)
            {
                ...
                if (counter > 1 && userToUserList.Contains(userParti)) //check if this user answer on this decition and took "wrong" answer
                {
                    counter = counter - 1;
                    dr["numOfCurrectAnswer"] = Convert.ToString(counter);
                }
        ...
        public int InsertUserToUser()
        {
            DBservices dbsLike = new DBservices();
            int numAffected = dbsLike.InsertUserToUserBL(this);
            return numAffected;
        }
